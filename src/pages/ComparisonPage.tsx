...
export function ComparisonPage() {
  ...
  return <section className="content-page comparison-page"><PageHeader title="Comparar plantas" subtitle="Compare os cuidados das duas plantas selecionadas." />{hasProductsUnderReview && <p className="knowledge-review">Informações específicas ainda estão em revisão.</p>}<div className="comparison-result-actions"><TouchButton variant="secondary" onClick={() => setComparing(false)}>Alterar seleção</TouchButton></div><div className="comparison-products">{selectedProducts.map((product) => <Link key={product.id} to={`/produto/${product.id}`}><CatalogProductImage src={product.image} alt="" /><div><span>{product.categoryLabel}</span><strong>{product.name}</strong></div></Link>)}</div><section className="comparison-facts" aria-label="Cuidados comparados">{comparisonFields.map(([label, field]) => {
